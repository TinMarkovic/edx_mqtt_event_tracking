...
class MapperTestCase(TestCase):
    ...
    def test_reading_event(self):
        event = self.events["book"]
        event = self.mapper.parse(event)
        self.assertIsInstance(event, caliper.events.ReadingEvent)
        self.assertEqual(event.action, caliper.profiles.ReadingProfile.Actions['VIEWED'])
